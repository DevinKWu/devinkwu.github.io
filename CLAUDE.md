...
| `throwCount` | `number` | 一次擲幾次（1–10000，預設 1） |
...
- **N>1，N ≤ DETAIL_LIMIT（50）**：逐筆清單（第N擲 + mini-block 標籤）+ 統計 bar + 連線紀錄
...
歷史顯示：單擲顯示 mini-block pair，批次顯示每擲標籤（有 `throws` 時）+ 統計 + 連線。祈求文字以 `<details>` 折疊。
